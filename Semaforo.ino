...
void setup() {
  ...
  setupPins();  // Configura los pines de los LEDs
  setupCamera();  // Inicializa la cámara
  connectToWiFi();  // Conecta a la red WiFi
  setupServer();  // Configura el servidor web
}
...
void loop() {
  ...
  handleSemaforo();  // Lógica del semáforo
  checkEmergencia();  // Verifica si hay una emergencia
  maintainLedState();  // Mantiene el estado del LED verde
}
...
// ==================
// MANTENIMIENTO DEL ESTADO DEL LED VERDE
// ==================
void maintainLedState() {
